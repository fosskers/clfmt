...
Transducers which themselves consume other transduction chains as arguments.
...
=branch=: Split a transduction chain based on a predicate. This produces a kind of
diamond pattern of data flow within the transduction:
...
Assuming that the top branch is some composition of three transducer steps, and
the bottom branch is a composition of two. Naturally, if you have other steps
beyond the fork point (Step 7 above), you must ensure that they can handle the
return values of both sides.
...
#+RESULTS:
: ("Waiting" "YRGNUH" "NIHTIW" "The" "Toronto" "Airport")
